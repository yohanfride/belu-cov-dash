...
class tes extends CI_Controller {
    ...
    public function add(){ 
		...
		if($this->input->post('save')){
			...
			$this->form_validation->set_rules('name', 'Nama Tes', 'required');
		...
		$data['jenis_status'] = ["Tes PCR","Tes TCM","Tes Reagen Antigen"];
	...
	public function edit($id){  
		...
		if($this->input->post('save')){
			$input=array(
				...
				// 'loc_lat' => $this->input->post('loc_lat'),
				// 'loc_long' => $this->input->post('loc_long'),
				'tgl_tesasi1' => $this->input->post('tgl_tesasi1'),
				'tgl_tesasi2' => $this->input->post('tgl_tesasi2')
			);
			$respo = $this->tes_m->edit($id,$input);
			if($respo->is_success){				
		...
		$data['data'] = $this->tes_m->get_detail($id)->data;
		if(!$data['data']){
		...
		$data['kecamatan'] = json_decode($json,true);
		$data['user_now'] = $this->session->userdata('covid-admin');					
		$this->load->view('tes_edit_v', $data);		
	}
    ...
    public function index(){        
		...
		if($this->input->get('jenis')){
			...
			$query['jenis'] = $data['grup'];
			$query2['jenis'] = $data['grup'];
		}
		$data['jenis_status'] = ["Tes PCR","Tes TCM","Tes Reagen Antigen"];
		...
		$data['data'] = $this->tes_m->search($query)->data;
		$data['total'] = $this->tes_m->search_count($query2)->data;
		$data['pagination'] = $this->tes_m->page($data['total'],$limit,$hal);
	...
	public function maps(){        
		...
		// 	echo '</pre>';
		// 	exit();
		$this->load->view('tes_maps_v', $data);
	}
	...
	public function get_data_tes(){
		...
		if($this->input->get('grup')){
			$query['jenis'] = $this->input->get('grup');
		}
	...
	public function grafik(){
		...
		// echo "</pre>";
		// exit();
		$this->load->view('tes_graph_v', $data);
	}
	...
	public function grafik2(){
		...
		// echo "</pre>";
		// exit();
		$this->load->view('tes_graph_v2', $data);
	}
	...
	public function import(){
		...
		$data['kecamatan'] = json_decode($json,true);
		$data['user_now'] = $this->session->userdata('covid-admin');					
   		$this->load->view('tes_import_v', $data);	
	}
	...
	function excel_format_puskesmas($sheet, $start_row, $end_row, $kode_import,$loc,$lng){
		...
		foreach($sheet as $row){
            if($numrow >= $start_row){
				$item['jenis'] = $row['J']; 
				if(strtolower($item['jenis']) == 'tes pcr'){
					$item['jenis'] = 'Tes PCR';
				}
				if(strtolower($item['jenis']) == 'tes tcm'){
					$item['jenis'] = "Tes TCM";
				}
				if(strtolower($item['jenis']) == 'tes reagen antigen'){
					$item['jenis'] = "Tes Reagen Antigen";
				}
				$item['nama'] = $row['C'] ;
				...
				if($row['L'])
					$item['tgl_tes'] = date("Y-m-d",strtotime(str_replace('/', '-', $row['L'])));
				else 
					$item['tgl_tes'] = '';

				if( strtolower($row['M']) == 'positif' || strtolower($row['M']) == 'reaktif' || strtolower($row['M']) == 'positif / reaktif' || strtolower($row['M']) == 'positif/reaktif')
					$item['hasil'] = 'Positif / Reaktif';
				else 
					$item['hasil'] = 'Negatif / Non-Reaktif';
				$cek_kecamatan = $this->cekkecamatan($item['kecamatan']);
				$cek_kelurahan = $this->cekkecamatan($item['kecamatan'],$item['kelurahan']);
				// echo $row['C'].' '.$row['B'].' '.$row['K'].' - '.$item['kecamatan'].' - '.$item['kelurahan'].' - '.$item['date_add'].' - '.$item['tgl_tes'].' - '.$item['hasil'].' kecamatan'.$cek_kecamatan.' -- kelurahan'.$cek_kelurahan.'<br/>';
	...
	public function delete($id=''){					
		//if(!$this->antrian_m->cek_hapus_g($data->NIP)){
			$del=$this->tes_m->del($id);
			if($del->is_success){
				redirect(base_url().'tes/?alert=success') ; 			
			} 
		//}
		redirect(base_url().'tes/?alert=failed') ; 			
	}
